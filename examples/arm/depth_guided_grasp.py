...
3. REACH   按实测指尖工具偏移计算 hover -> engage 路径点并移动左腕；
...
BASE_CORRECTION_M = (-0.012, 0.048, 0.053)    # 卷尺真值比对（右臂值 Y 镜像，未独立复测）
TOOL_OFFSET_M = (0.228, -0.025, -0.037)       # 指尖 - 手腕，腕部水平姿态（右臂值 Y 镜像，未独立复测）
...
# 指令的腕部目标必须保持在这个盒内。
WRIST_BOX = ((-0.05, 0.70), (-0.25, 0.60), (0.60, 1.35))  # 右臂盒 Y 镜像，未独立复测
...
def project_root() -> Path:
    ...
    return here.parent.parent if here.name == "arm" else here.parent
...
def load_gripper_calibration(root: Path) -> tuple[float, float, float]:
    ...
    config_path = root / "bottle_grasp.json"
    try:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Head-down -> depth-camera locate -> left-arm grasp flow. "
...
class Bus:
    ...
    def left_gripper(self) -> float:
        status = self.wait_fresh("latest_status")
        return float(status["left_gripper_state"]["position"][0])
...
def move_left_wrist(bus: Bus, target, left_quat) -> None:
    while True:
        (left_pos, _), (right_pos, right_quat) = bus.eef_state()
        remaining = math.dist(left_pos, target)
        if remaining <= SETTLE_NEAR_M:
            break
        scale = min(1.0, STEP_MAX_M / remaining)
        chunk = [lp + (tv - lp) * scale for lp, tv in zip(left_pos, target)]
        bus.publish(bus.move_pub, {
            "pos_left_in_robot": chunk, "quat_left_in_robot": left_quat,
            "pos_right_in_robot": right_pos, "quat_right_in_robot": right_quat,
        }, "left wrist target")
        wait_settled(bus, chunk)


def command_gripper(bus: Bus, position: float, label: str) -> None:
    bus.publish(bus.grip_pub, {"left_gripper_target_joints_position": [position]},
...
def main(argv=None) -> int:
    ...
    robot = os.uname().nodename.rsplit("-", 1)[-1]  # 机器编号一律按主机名推导，不信环境变量
    ...
    try:
        ...
        print(f"[plan] left wrist at ({left_pos[0]:.3f}, {left_pos[1]:.3f}, {left_pos[2]:.3f})")
        legs = plan_waypoints(bottle, left_pos)
        for name, target in legs:
        ...
        for name, target in legs:
            ...
            if name == "open-gripper":
                ...
                move_left_wrist(bus, target, left_quat)
        return EXIT_OK
