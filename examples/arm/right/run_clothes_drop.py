...
try:  # 从项目根目录按包导入。
    from examples.arm.right.clothes_drop_config import (
        STEP_MAX_M,
        ConfigError,
        load_config,
        require_identity,
...
def _preview_text(config_path: str) -> str:
    lines = [
        ...
        "  precondition: right gripper feedback must sit inside the calibrated cloth-holding band",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--config", default="right/clothes_drop.json", help="calibration configuration path")
