...
_ROOT = _HERE.parent.parent if _HERE.name == "arm" else _HERE.parent
for _path in (str(_ROOT), str(_HERE)):
    if _path not in sys.path:
        sys.path.insert(0, _path)

try:  # 从项目根目录按包导入。
    from examples.arm.bottle_grasp_config import (
        BACK_CLEARANCE_M,
...
# 旋转误差门：原 1°。2026-08-22 robot-260 左臂伺服中 daemon 已到位（关节差 <0.02°、
# 位置差 0.1 mm）但 EEF 姿态较上一步反馈偏 1–2°（IK 在向中线伸展构型下的姿态解偏差），
# 1° 门永不收敛（run 043022Z）；放宽到与接触门相同的 3°，位置门与各限位不变。
ROTATION_FEEDBACK_MAX_DEG = 3.0
...
EXPECTED_CAMERA_ID = "mod_camera_hand_left"

STAGES = (
    "PREFLIGHT",        # 预检：读实时状态，左臂 X 必须在批准区间内
    "OPEN_GRIPPER",     # 张爪到 10 并等反馈确认
    "BACK_CLEARANCE",   # 后退 BACK_CLEARANCE_M（当前 0），实际到达位置即"观察锚点"
...
def parse_gripper_state(value: Any) -> GripperState:
    ...
    if isinstance(value, GripperState):
        return value
    root = _value(value, "left_gripper_state")
    if root is None:
        root = value
    position = _value(root, "position", "positions")
    if isinstance(position, Sequence) and not isinstance(position, (str, bytes, bytearray)):
        if len(position) != 1:
            raise RunnerError("left_gripper_state.position must contain one value")
        position = position[0]
    if position is None:
        raise RunnerError("gripper feedback is missing left_gripper_state.position")
...
def move_left(state: EEFState, *, dx: float = 0.0, dy: float = 0.0, dz: float = 0.0) -> EEFState:
    left = state.left
    return EEFState(
        Pose(
            (left.position[0] + dx, left.position[1] + dy, left.position[2] + dz),
            left.rotation,
        ),
        state.right,
    )
...
def set_left_z(state: EEFState, z_value: float) -> EEFState:
    left = state.left
    return EEFState(Pose((left.position[0], left.position[1], z_value), left.rotation), state.right)
...
# 双臂反馈门：左右两侧的位置/旋转误差必须同时进入容差带。未选侧（右臂）
# 也受门约束，可以及早发现"只想动左臂却把右臂带动了"的异常。
def eef_target_reached(
    ...
    *,
    ...
    # 右臂是透传侧：目标恒为启动快照，daemon 双臂联合解 IK 与导航振动会让它
    # 漂移几毫米（2026-08-22 robot-260 flow 054202Z：左臂位置/姿态/关节全部到位，
    # 右臂离快照 4.6 mm，3 mm 门永不收敛）。它的门只需发现"明显被带动"，
    # 取宽容门（2 cm / 3°）；受控的左臂保持严格门。
    passive_position = max(position_tolerance_m, LOADED_POSITION_FEEDBACK_MAX_M)
    passive_rotation = max(rotation_tolerance_deg, LOADED_ROTATION_FEEDBACK_MAX_DEG)
    return (
        _position_error(actual.right, target.right) <= passive_position + BOUNDARY_TOLERANCE
        and _rotation_error_degrees(actual.right, target.right) <= passive_rotation + BOUNDARY_TOLERANCE
...
def run_grasp(
    ...
    *,
    ...
    try:
        # 阶段 1 预检：读一帧实时双臂状态；左臂 X 不在批准区间说明复位
        # 异常或姿态不对，一步都不动直接拒跑。
        record_stage("PREFLIGHT")
        state = parse_eef_state(runtime.read_eef_state())
        if not config.x_absolute_min_m <= state.left.position[0] <= config.x_absolute_max_m:
            return abort(
                f"preflight left X {state.left.position[0]:.4f} is outside the approved "
        ...
        back_target = move_left(state, dx=-BACK_CLEARANCE_M)
        if back_target.left.position[0] < config.x_absolute_min_m:
            return abort("back clearance would leave the approved X range")
        anchor = move_and_wait(back_target)
        anchor_x = anchor.left.position[0]
        anchor_y = anchor.left.position[1]
        ...
        if config.grasp_z_m < anchor.left.position[2] - GRASP_Z_RELATIVE_FLOOR_M:
            return abort(
                f"grasp height {config.grasp_z_m:.3f} is below the -{GRASP_Z_RELATIVE_FLOOR_M} m floor "
                f"relative to the anchor height {anchor.left.position[2]:.3f}"
            )
        current = move_and_wait(set_left_z(anchor, config.grasp_z_m))
        ...
        while True:
            ...
            # 例外：向中线（左臂为 -Y）的可达空间受 daemon 自碰撞门约束，
            # 实测约与前伸量 1:1 增长（右臂 +Y 方向实测值按镜像沿用）；
            # 预算不足时不中止，改为落入下方 X 前进分支，
            # 先扩大工作空间，下一轮再修 Y。
            if not centered:
                ...
                midline_budget_m = MIDLINE_Y_PER_FORWARD_RATIO * (current.left.position[0] - anchor_x)
                midline_deferred = (
                    step < 0
                    and current.left.position[1] - anchor_y + step < -(midline_budget_m + BOUNDARY_TOLERANCE)
                )
                if not midline_deferred:
                    current = move_and_wait(move_left(current, dy=step))
                    y_net += step
                    continue
            ...
            forward_from_anchor = current.left.position[0] - anchor_x
            ...
            if current.left.position[0] + step > config.x_absolute_max_m + BOUNDARY_TOLERANCE:
                return abort(f"X advance would leave the approved absolute range (max {config.x_absolute_max_m} m)")
            current = move_and_wait(move_left(current, dx=step))
        ...
        if config.final_forward_m > 0.0:
            ...
            if current.left.position[0] + config.final_forward_m > config.x_absolute_max_m + BOUNDARY_TOLERANCE:
                return abort("final forward would leave the approved absolute X range")
            # 用默认门（位置 3 mm）等前伸真正走完再闭爪：接触门的 2 cm 位置容差会让
            # 爪子在前伸未完成时提前闭合（2026-08-22 现场观察）。姿态偏差问题由
            # ROTATION_FEEDBACK_MAX_DEG=3° 覆盖。
            current = move_and_wait(move_left(current, dx=config.final_forward_m))
        ...
        current = move_and_wait(move_left(current, dz=LIFT_M), loaded=True)
...
class _RosRuntime:
    ...
    def __init__(self, names: Mapping[str, str], evidence_dir: Any) -> None:
        ...
        self._mode_client = self._node.create_client(SetParameters, self.names["left_mode_service"])
    ...
    # 实跑前接口预检：确认两个目标话题都有订阅者，并通过参数服务把左爪
    ...
    def preflight_interfaces(self) -> None:
        ...
        if not self._mode_client.wait_for_service(timeout_sec=DEFAULT_FEEDBACK_TIMEOUT_SECONDS):
            raise RunnerError(f"position-mode service unavailable: {self.names['left_mode_service']}")
        ...
        self._spin_until(lambda: future.done(), DEFAULT_FEEDBACK_TIMEOUT_SECONDS, "left position mode")
        response = future.result()
        results = getattr(response, "results", None)
        if not results or not bool(getattr(results[0], "successful", False)):
            raise RunnerError("left position-mode service rejected the request")
    ...
    def publish_gripper_target(self, target: float) -> None:
        ...
        message = self._string_type()
        message.data = _json_text({"left_gripper_target_joints_position": [float(target)]})
    ...
    def acquire_stable(self, config: BottleGraspConfig) -> Any:
        try:
            ...
            from examples.camera.read_hand_camera import LeftHandDecodedCamera
        ...
        if self._camera is None:
            # 260 上厂商 decoded 管线不出帧（.codex/issues.md 2026-08-22），
            # 改读 JPEG 目录输出并本地解码；返回值形态不变。
            self._camera = LeftHandDecodedCamera(timeout=2.0, source="jpeg")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Lean left-hand bottle visual-servo grasp.")
    ...
    parser.add_argument("--config", default="bottle_grasp.json", help="calibration configuration path")
