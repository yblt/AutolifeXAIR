...
安全底线：实机执行需要 ``run --execute``；左爪反馈证明确实持物
...
try:  # 从项目根目录按包导入。
    from examples.arm.basket_drop_config import (
        STEP_MAX_M,
        BasketDropConfig,
        ConfigError,
        load_config,
...
# 反馈门、话题名和错误类型。
_ARM = _load_sibling("arm", "run_bottle_grasp")
...
def _with_left_position(state: EEFState, position: Sequence[float]) -> EEFState:
    left = state.left
    return EEFState(Pose(tuple(float(value) for value in position), left.rotation), state.right)
...
def run_drop(
    ...
    *,
    ...
    try:
        ...
        if not config.contains(state.left.position):
            return abort(
                f"preflight left EEF position {list(state.left.position)} is outside the "
        ...
        if grip.communication_lost:
            return abort("left gripper feedback reports communication_lost; refusing to execute")
        band_error = abs(grip.position - config.held_feedback_center)
        if band_error > config.held_feedback_tolerance + BOUNDARY_TOLERANCE:
            return abort(
                f"left gripper feedback {grip.position:.1f} is outside the holding band "
        ...
        for waypoint in plan_axis_steps(
            state.left.position, hover, config.step_max_m, y_align_x_m=config.y_align_x_m
        ):
            ...
            current = move_and_wait(_with_left_position(current, waypoint), loaded=True)
        ...
        for waypoint in plan_axis_steps(
            current.left.position, config.retract_position_m, config.step_max_m, y_align_x_m=config.y_align_x_m
        ):
...
def _preview_text(config_path: str) -> str:
    lines = [
        ...
        "  precondition: left gripper feedback must sit inside the calibrated holding band",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--config", default="basket_drop.json", help="calibration configuration path")
