...
try:  # 从项目根目录按包导入。
    from examples.arm.clothes_grasp_config import (
        ClothesGraspConfig,
        ConfigError,
        load_config,
        require_identity,
    ...
    *,
    ...
    try:
        ...
        def execute_path(goal: Sequence[float], step: float, context: str) -> None:
            nonlocal state
            for waypoint in plan_axis_steps(state.left.position, goal, step):
                ...
                violation = check_table_rules(state.left.position, waypoint, config)
                if violation is not None:
                    raise RunnerError(f"table-safety refusal during {context}: {violation}")
                state = move_and_wait(_with_left_position(state, waypoint))
        ...
        current_quat = tuple(float(value) for value in state.left.rotation)
        ...
        if posture_angle > ROTATION_DONE_TOLERANCE_DEG:
            ...
            for index in range(1, rotation_steps + 1):
                ...
                state = move_and_wait(
                    EEFState(Pose(state.left.position, stepped), state.right)
                )
        ...
        for attempt in (0, 1):
            ...
            execute_path((state.left.position[0], state.left.position[1], config.pinch_z_m),
            ...
            try:
                ...
                execute_path(
                    (state.left.position[0], state.left.position[1], align_z),
                    config.descend_step_m,
                    "the retry ascent",
                )
                retry_point = (
                    state.left.position[0] + config.retry_offset_m[0],
                    state.left.position[1] + config.retry_offset_m[1],
                    align_z,
                )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--config", default="clothes_grasp.json", help="calibration configuration path")
